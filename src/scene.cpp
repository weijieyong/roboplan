...
namespace roboplan {
    ...
    Scene::Scene(const std::filesystem::path& urdf_path, const std::filesystem::path& srdf_path)
    {
        ...
        pinocchio::urdf::buildGeom(model_, urdf_path, pinocchio::VISUAL, visual_model, package_paths);

        pinocchio::GeometryModel collision_model;
        pinocchio::urdf::buildGeom(model_, urdf_path, pinocchio::COLLISION, collision_model, package_paths);
        collision_model.addAllCollisionPairs();
        pinocchio::srdf::removeCollisionPairs(model_, collision_model, srdf_path);

        std::cout << "Built a Pinocchio model with " << std::to_string(model_.nq) << " DOFs\n";
